...
def backfill_text(summaries: list[BackfillResult]) -> str:
    ...
    for summary in summaries:
        failed_users = backfill_failed_users_text(summary, limit=500)
        failed_user_lines = "\n".join(
            f"> {line}" for line in failed_users.splitlines()
        )
        lines.append(
...
def backfill_failed_users_text(summary: BackfillResult, *, limit: int = 1200) -> str:
    ...
    for index, failed_user in enumerate(summary.failed_users):
        candidate_lines = [*lines, f"- {failed_user}"]
        remaining_count = len(summary.failed_users) - index - 1

        suffix = f"\n...and {remaining_count} more." if remaining_count else ""
    ...
    if not lines:
        return f"{len(summary.failed_users)} users failed; list is too long to display here."
