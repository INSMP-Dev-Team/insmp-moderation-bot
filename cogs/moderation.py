    ...
    async def backfill_bans(
            ...
            for ban_entry in home_bans:
                ...
                try:
                    ...
                    summary.failed_users.append(
                        f"<@{user.id}> {user} (`{user.id}`): missing permissions or role hierarchy"
                    )
                except discord.HTTPException as exc:
                    summary.failed += 1
                    summary.failed_users.append(
                        f"<@{user.id}> {user} (`{user.id}`): Discord API error: `{shorten_text(exc, 160)}`"
                    )
        ...
        await ctx.reply(
            (
                ...
                f"Failed users:\n{backfill_failed_users_text(summary, limit=900)}"
            ),
            mention_author=False,
